...
├── redeploy.ps1                  # ビルド→テスト→常駐停止→publish→再起動を一括実行
├── SPEC.md                       # 本仕様書
├── src/
│   └── CodexChecker/
...
| 位置 | プライマリ画面 WorkingArea の右下角にぴったり合わせる |
| TopMost | true |
| ShowInTaskbar | false |
| フォーカス | 奪わない (`ShowWithoutActivation = true`)。作業中に出ても入力を妨げない |
| サイズ | 約 500 x 142 px (相対時間のリセット表示が見切れない範囲で抑える) |
...
```
Codex                                  x
5h ▓▓▓▓▓▓▓▓░░ 残り80%　あと4時間37分でリセット
1w ▓▓▓▓▓░░░░░ 残り52%　2026-07-12 09:05
Claude Usage を開く        ← `Usage を開く` だけクリックでブラウザ起動
```
...
- rateLimits に `resetsAt` / `resetsInSeconds` 相当のフィールドがあれば
  リセット情報を併記する (無ければ省略)
  - 5h はリセットまでの相対時間「残りN%　あとX時間Y分でリセット」。
    24 時間以上なら「あとX日Y時間でリセット」、1 時間未満なら
    「あとY分でリセット」(切り上げ)、経過済みなら「まもなくリセット」
  - 1w はリセット時刻「残りN%　yyyy-MM-dd HH:mm」
- `Claude` は通常テキスト、`Usage を開く` はリンクラベルとし、クリックで
...
| トリガー | 動作 |
|----------|------|
| 起動直後 / 10 分タイマー | 最新値を取得して表示 → **10 秒後に自動で消える** |
| `Ctrl + Alt + X` | 非表示なら表示して 10 秒後に自動で消える。表示中なら閉じる |
| バツボタン | 閉じる |
| ポップアップを左クリック | 閉じる |
| 右クリック | コンテキストメニュー表示: 「再取得」「常駐を終了」 |

- 定期表示で出ている間にホットキーを押した場合も閉じる
- 表示経路に関わらず、表示後は 10 秒で自動的に閉じる
